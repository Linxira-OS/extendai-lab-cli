...
func TestInstallerCommandPassesUnquotedDFlagLast(t *testing.T) {
	cmd := installerCommand(`C:\Temp\extendai-lab-update-1.exe`, `D:\Tools\extendai-lab App`)
	if cmd.SysProcAttr == nil {
		t.Fatal("expected a raw command line forcing the install dir")
	}
	got := cmd.SysProcAttr.CmdLine
	want := `"C:\Temp\extendai-lab-update-1.exe" /D=D:\Tools\extendai-lab App`
	if got != want {
...
func TestInstallerCommandWithoutDirSkipsDFlag(t *testing.T) {
	cmd := installerCommand(`C:\Temp\extendai-lab-update-1.exe`, "")
	if cmd.SysProcAttr != nil {
