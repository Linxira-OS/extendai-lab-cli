...
// Trust gates project hooks. A project's .extendai-lab/settings.json can run
...
// root. The trust flag lives in user-global state (~/.extendai-lab/trust.json),
// NOT in the project file itself — an attacker controls the latter. Global
// hooks (~/.extendai-lab/settings.json) are the user's own and always run.

// TrustFilename is the user-global trust store under ~/.extendai-lab.
...
// TrustPath is ~/.extendai-lab/trust.json (homeDir overrides ~).
