...
// (.extendai-lab/settings.json, only when the project is trusted) and a global
// (~/.extendai-lab/settings.json) file. A hook's exit
...
const (
	SettingsDirname  = ".extendai-lab"
	SettingsFilename = "settings.json"
)

// GlobalSettingsPath is ~/.extendai-lab/settings.json (homeDir overrides ~).
func GlobalSettingsPath(homeDir string) string {
	return filepath.Join(home(homeDir), SettingsDirname, SettingsFilename)
}

// ProjectSettingsPath is <root>/.extendai-lab/settings.json.
