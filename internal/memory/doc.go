// Package memory implements extendai-lab's persistent memory. It mirrors Claude
// Code's two-layer model while honoring extendai-lab's cache-first architecture:
//
//   - Hierarchical doc memory: extendai-lab.md / AGENTS.md files discovered from the
...
const (
	ScopeUser     Scope = "user"     // ~/.config/extendai-lab/extendai-lab.md
	ScopeAncestor Scope = "ancestor" // a extendai-lab.md above the project root
	ScopeProject  Scope = "project"  // ./extendai-lab.md (committed, shared)
	ScopeLocal    Scope = "local"    // ./extendai-lab.local.md (personal, git-ignored)
)

// docNames are the recognized memory filenames at each level, in load order.
// extendai-lab.md is ours; AGENTS.md and CLAUDE.md are the cross-tool conventions.
...
var docNames = []string{"extendai-lab.md", "AGENTS.md", "CLAUDE.md"}

// localNames are the personal, git-ignored overrides, highest precedence.
var localNames = []string{"extendai-lab.local.md", "AGENTS.local.md", "CLAUDE.local.md"}
