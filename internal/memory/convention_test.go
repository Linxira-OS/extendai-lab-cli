...
func TestDocPathPrefersExisting(t *testing.T) {
	proj := t.TempDir()
	// An existing extendai-lab.md should keep receiving notes (no split to AGENTS.md).
	if err := os.WriteFile(filepath.Join(proj, "extendai-lab.md"), []byte("x"), 0o644); err != nil {
		t.Fatal(err)
	}
	set := Load(Options{CWD: proj})
	if got := set.DocPath(ScopeProject); filepath.Base(got) != "extendai-lab.md" {
		t.Errorf("should append to the existing extendai-lab.md, got %s", got)
	}
