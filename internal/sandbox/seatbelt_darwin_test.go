...
func TestSandboxEnforcesWrites(t *testing.T) {
	...
	workRoot, err := os.MkdirTemp(home, ".extendai-lab-sbtest-work-*")
	if err != nil {
		t.Skipf("cannot create work dir under home: %v", err)
	}
	t.Cleanup(func() { os.RemoveAll(workRoot) })
	outside, err := os.MkdirTemp(home, ".extendai-lab-sbtest-out-*")
	if err != nil {
...
func TestGoBuildUnderSandbox(t *testing.T) {
	...
	work, err := os.MkdirTemp(home, ".extendai-lab-sbtest-go-*")
	if err != nil {
		t.Skipf("cannot create work dir under home: %v", err)
	}
