...
// $LC_ALL / $extendai-lab_LANG. Unknown locales return "" so DetectLanguage falls
...
// TestDetectLanguagePriority verifies override beats env and that extendai-lab_LANG
// beats LANG. With a clean env we fall back to English.
func TestDetectLanguagePriority(t *testing.T) {
	t.Setenv("extendai-lab_LANG", "")
	t.Setenv("LC_ALL", "")
	...
	t.Setenv("extendai-lab_LANG", "en")
	if got := DetectLanguage(""); got != "en" {
		t.Errorf("extendai-lab_LANG=en overriding LANG=zh: got %q, want en", got)
	}
