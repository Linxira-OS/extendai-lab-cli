...
type Messages struct {
	...
	StepScaffold    string // step 1 desc — extendai-lab setup
	StepSetKey      string // step 2 command label

	// `extendai-lab init` — points to the in-session /init skill + setup
	InitHint       string
	StepSetKeyHint string // step 2 desc — env var hint
	StepChatDesc   string // extendai-lab chat step desc
	StepRunDesc    string // extendai-lab run step desc
	HelpFooter     string // dim footer linking to extendai-lab help
	...
	WroteFileFmt          string // "Wrote %s" — used for extendai-lab.toml and .env both
	...
	SkipStaleCustomEntryFmt    string // "skipping stale %q entry from extendai-lab.toml (pointing at %s) — please remove it"
	...
	UsageRunHint              string // "usage: extendai-lab run [--model NAME] <task>"
...
//
// Priority: override > extendai-lab_LANG > LC_ALL > LC_MESSAGES > LANG > "en".
...
func envCandidates() []string {
	keys := []string{"extendai-lab_LANG", "LC_ALL", "LC_MESSAGES", "LANG"}
