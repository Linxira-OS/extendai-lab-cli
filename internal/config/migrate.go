...
// legacyConfig is the subset of the v0.x (~/.extendai-lab/config.json) schema this
// import carries forward. Fields absent here are dropped on purpose: desktop tab
// state is frontend-owned, and skills already live in the shared ~/.extendai-lab/skills
...
func (r *MigrationResult) Notice() string {
	...
	if r.KeyToEnv {
		b.WriteString("; API key saved to extendai-lab's credentials store")
	}
...
// checks v1-era TOML first, then v0.5/v0.x ~/.extendai-lab/config.json, and never
...
func MigrateLegacyIfNeeded() (*MigrationResult, error) {
	...
	src := filepath.Join(home, ".extendai-lab", "config.json")
...
func legacyTOMLPaths(dest, home string) []string {
	paths := []string{filepath.Join(filepath.Dir(dest), "extendai-lab.toml")}
	if home != "" {
		paths = append(paths, filepath.Join(home, ".extendai-lab", "extendai-lab.toml"))
	}
	return paths
}
...
// writeCredentialsEnv merges lines into the extendai-lab-owned global credentials
// file (UserCredentialsPath, e.g. %AppData%\extendai-lab\credentials), replacing any
