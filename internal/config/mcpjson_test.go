...
command = "local-bin"
`
	if err := os.WriteFile("extendai-lab.toml", []byte(toml), 0o644); err != nil {
		t.Fatal(err)
	}
	mcp := `{ "mcpServers": {
  ...
  "extra":  { "command": "extra-bin", "auto_start": false }
	...
	if byName["shared"].Command != "local-bin" || byName["shared"].URL != "" {
		t.Errorf("extendai-lab.toml should win the collision, got %+v", byName["shared"])
	}
...
func TestLoadMergesPluginsAcrossTOMLSources(t *testing.T) {
	...
	if err := os.WriteFile("extendai-lab.toml", []byte("[[plugins]]\nname = \"projectmcp\"\ncommand = \"project-bin\"\n"), 0o644); err != nil {
		t.Fatal(err)
	}
	...
	if !names["globalmcp"] || !names["projectmcp"] {
		t.Fatalf("a project extendai-lab.toml [[plugins]] dropped the global config's server; got %+v", cfg.Plugins)
	}
}
...
func TestMergeMCPJSONPrecedence(t *testing.T) {
	// extendai-lab.toml already declares "shared" (stdio); .mcp.json offers a colliding
	// "shared" (http) plus a fresh "extra". extendai-lab.toml must win on the collision;
	// "extra" gets appended.
	...
	if cfg.Plugins[0].Name != "shared" || cfg.Plugins[0].Command != "local-bin" || cfg.Plugins[0].URL != "" {
		t.Errorf("collision not won by extendai-lab.toml: %+v", cfg.Plugins[0])
	}
...
name = "dida"
type = "http"
url = "https://extendai-lab.example/mcp?access_token=toml"
[plugins.headers]
...
`), 0o644); err != nil {
  ...
  "dida": {
	...
	if source != "extendai-lab.toml" {
		t.Fatalf("source = %q, want extendai-lab.toml", source)
	}
	if updated.URL != "https://extendai-lab.example/mcp" {
		t.Fatalf("updated URL = %q", updated.URL)
	}

	projectRaw, err := os.ReadFile("extendai-lab.toml")
	if err != nil {
		t.Fatal(err)
	}
	if strings.Contains(string(projectRaw), "access_token=toml") || strings.Contains(string(projectRaw), "Authorization") {
		t.Fatalf("extendai-lab.toml auth material should be removed:\n%s", projectRaw)
	}
