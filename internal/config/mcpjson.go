...
// legacyConfigPath is the v0.x (TypeScript line) config file, ~/.extendai-lab/config.json.
func legacyConfigPath() string {
	home, err := os.UserHomeDir()
	if err != nil {
		return ""
	}
	return filepath.Join(home, ".extendai-lab", "config.json")
}

// loadLegacyMCP reads the v0.x ~/.extendai-lab/config.json and returns its enabled
...
// already declare. extendai-lab.toml's [[plugins]] win on a name collision: it is the
