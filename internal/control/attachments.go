...
// .extendai-lab/attachments and returns its repo-relative path for @referencing.
...
func cleanAttachmentPath(path string) (string, error) {
	...
	root := filepath.Join(".extendai-lab", "attachments")
	if clean == "." || clean == root || strings.HasPrefix(clean, ".."+string(filepath.Separator)) || !strings.HasPrefix(clean, root+string(filepath.Separator)) {
		return "", fmt.Errorf("attachment path is outside .extendai-lab/attachments")
	}
	if err := ensureAttachmentRoot(); err != nil {
		return "", err
	}
...
func attachmentPath(ext string) string {
	...
	return filepath.Join(".extendai-lab", "attachments", name)
}
