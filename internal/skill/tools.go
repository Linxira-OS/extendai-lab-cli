...
func (t *installSkillTool) Description() string {
	scope := "'global' (only option — no project workspace) writes to ~/.extendai-lab/skills/."
	if t.store.HasProjectScope() {
		scope = "'project' (default) writes to <repo>/.extendai-lab/skills/ (this workspace only); 'global' writes to ~/.extendai-lab/skills/ (every project)."
	}
...
"type":"object",
"properties":{
  "name":{"type":"string","description":"Identifier — letters/digits/_/-/., 1-64 chars, starts alphanumeric. Becomes the skill folder name under ~/.extendai-lab/skills/<name>/SKILL.md."},
